...
brute_force_nn(const std::vector<PointType>& data, const PointType& query) {
...
template <typename PointType>
std::vector<PointType> brute_force_knn(
    const std::vector<PointType>& data, const PointType& query, std::size_t k
) {
    std::vector<std::pair<decltype(kdtree::squared_euclidean_distance(query, data[0])), std::size_t>> dists;
...
TEST_CASE("nnsearch_kdtree matches brute force for multiple queries", "[kdtree][nn]") {
    ...
    std::vector<kdtree::point<int, 2>> queries = {
        {0, 0}, {5, 5}, {-10, -10}, {3, 7}, {-3, -3}
    };
...
TEST_CASE("nnsearch_kdtree with float points - pruning correctness", "[kdtree][nn][pruning]") {
    ...
    INFO("brute force NN:  (" << (*brute_it)[0] << ", " << (*brute_it)[1] << ") at squared dist " << brute_dist);
...
TEST_CASE("nnsearch_kdtree with float points - larger dataset", "[kdtree][nn][pruning]") {
    ...
    std::vector<kdtree::point<double, 2>> data = {
        {0.1, 0.2}, {0.5, 0.8}, {0.3, 0.1}, {0.9, 0.4}, {0.7, 0.6},
        {0.2, 0.9}, {0.4, 0.3}, {0.6, 0.7}, {0.8, 0.5}, {0.15, 0.85},
        {0.35, 0.55}, {0.65, 0.15}, {0.95, 0.95}
    };
    kdtree::make_kdtree(data.begin(), data.end());

    std::vector<kdtree::point<double, 2>> queries = {
        {0.25, 0.25}, {0.75, 0.75}, {0.0, 0.0}, {1.0, 1.0},
        {0.45, 0.45}, {0.11, 0.81}, {0.33, 0.07}
    };
...
TEST_CASE("radiusquery_kdtree basic", "[kdtree][radius]") {
    ...
    for (const auto& it : results) {
        auto dist = std::sqrt(
            static_cast<double>(kdtree::squared_euclidean_distance(center, *it))
        );
        REQUIRE(dist <= radius);
    }
}
